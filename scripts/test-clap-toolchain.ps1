...
$state = @{ Downloads = 0; BadHash = $false; DownloadFailures = 0; SleepSeconds = @() }
function Invoke-WebRequest($Uri, $OutFile, [switch]$UseBasicParsing) {
    ...
    $state.Downloads++
    if ($state.DownloadFailures -gt 0) {
...
function Start-Sleep([int]$Seconds) { $state.SleepSeconds += $Seconds }
...
try {
    ...
    if ($state.Downloads -ne 4) { throw "Expected one rejected download and three retry attempts, got $($state.Downloads)" }
    if (($state.SleepSeconds -join ",") -ne "2,4") { throw "Unexpected download retry delays: $($state.SleepSeconds -join ',')" }
    if (@(Get-ChildItem $dir -Filter ".llvm-install-*").Count -ne 0) { throw "Staging directory retained" }
    Write-Host "PASS: pinned download, bounded retries, checksum rejection, offline reuse, both targets, quoted paths and cleanup"
