...
try {
  ...
  await page.route("**/src/lib/api.ts", route => route.fulfill({ contentType: "application/javascript", body: `
    ...
    const emit=note=>window.dispatchEvent(new CustomEvent('playlistai:progress',{detail:{op:'analysis-model',done:123,total:793130000,note}}));
    const download=()=>{
    ...
    const methods={
      ...
      GetModelRecommendations:()=>({models:[{id:'qwen9',label:'Qwen3.5 9B',params:'9B',sizeApprox:5680522464,ramGb:8,recommended:true,installed:true}],hardware:{gpuAvailable:false}}),
      GetRecommendedAnalysisBundle:()=>bundle,
      GetAnalysisStatus:()=>({installed,available:false,enabled:false,model:bundle.label,detail:installed?'CLAP model installed and inference validated. Automatic musical-fit decisions await a calibrated policy.':'Download a CLAP model.',storage:{bytes:0,records:0},downloadBytes:793130000,memoryBytes:2147483648}),
  ...
  await page.getByText("CLAP model installed and inference validated.",{exact:false}).waitFor();
