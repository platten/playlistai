...
const fixture = `
let installed=false,enabled=false,attempts=0;
...
const methods={GetIntentAssistStatus:status,SetIntentAssistEnabled:v=>{enabled=v},InstallIntentModels:()=>{attempts++;if(attempts===1)return Promise.reject(new Error('Download interrupted'));installed=true}};
...
try {
  ...
  console.log('Intent setup UI passed: automatic download failure/retry, installed state, keyboard toggle, dark/light and narrow/wide, unsupported host without download.');
