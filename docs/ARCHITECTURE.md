...
streaming service via Soundiiz. Go + [Wails v2] backend, React + TypeScript
frontend.
...
FIRST LAUNCH (one-time, user-initiated, with progress bars):
  ...
  ───────────────────────────────────────────────────
  Playlist ──► Enricher (MusicBrainz: ISRC, album, year) ──► Exporter (Soundiiz API | CSV) ──► Qobuz
...
| `Exporter` | Send a playlist out — `soundiiz-api` (token) or `csv` (always available). |
...
```
main.go                     Wails shell (root, per Wails convention)
wails.json
internal/
  ...
  bridge/     Wails-bound API struct + WailsProgress (thin; no logic)
  catalog/    [M3] mmap int8 vectors + SQLite metadata; token search
  dataset/    [M3] first-launch download + verify of the converted catalog
  ...
  export/     [M7] soundiizcsv/ soundiizapi/
  preview/    [M8] deezer/ spotifycdn/
frontend/     Vite + React + TS  (design system + screens land in M2)
python/       convert_pickles.py, parity_playlist.py  (build-time tooling, not shipped)
...
required — cache path, min match score), `[export]` (Soundiiz token),
`[preview]` (`deezer` | `spotify` | `off`).

---

## 7. Testing

- `go test ./...` needs only the Go toolchain — no ONNX, no llama, no network.
...
1. **Skeleton** — layout, core types, ports + fakes, `Container`, config, Wails
   shell, `Progress` contract, CI (lint / test / cross-compile). *(current)*
2. **Design pass** — visual system + mockups; shared components (`ProgressBar`,
   `EmptyState`, `LoadingState`, `ErrorState`, `Slider`, `TrackRow`); theming.
3. **Catalog** — `convert_pickles.py`; int8 + SQLite loader; token search;
   first-launch download with resumable progress.
...
7. **Enrichment + export** — MusicBrainz client + cache; Soundiiz CSV + API;
   match-review screen.
