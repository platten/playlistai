...
frontend/     Vite + React + TS + @wailsio/runtime; pnpm; Tailwind v4 + Radix
  ...
  src/components/ ProgressBar (+ useProgress on the playlistai:progress event),
                  EmptyState, LoadingState, ErrorState, Slider, TrackRow, Button, icons
  src/screens/    [M3+] Generate, Playlist, ReviewExport, FirstRun, Settings
...
2. **Design pass** — mockup canvas; Tailwind v4 + Radix; `src/design/` tokens +
   ...
   `LoadingState`, `ErrorState`, `Slider`, `TrackRow`, `Button`). *(current)*
