...
MERT is separately licensed **CC-BY-NC-4.0**; ONNX Runtime is MIT. Windows packs
also carry Microsoft Visual C++ Runtime 14.51.36247 DLLs under their separate
Microsoft terms. The application remains GPL-3.0. Optional packs carry upstream notices and do not expand model
...
Original source assets remain intact. `derived-mert/packs-v2` holds the five final
pack directories/ZIPs and provenance inventories. The earlier `packs-v1` files
are retained as historical artifacts; their Windows packs lack app-local CRT
dependencies and are superseded. Original ONNX Runtime archives total 201,141,124
bytes; official Microsoft CRT source EXEs total 30,602,672 bytes. Final v2 ZIPs
total **1,195,419,380 bytes**. The worker verifies Windows dependency paths point
inside the checked pack, preventing reliance on preinstalled CRT DLLs.
...
worker peak working set with the final app-local CRT pack was 525,746,176 bytes;
it is not an all-platform bound.
...
| Python `python -m unittest discover -s python -p 'test_*mert*.py'` | PASS: 17 preprocessing, pack-safety and CRT preparation regressions |
