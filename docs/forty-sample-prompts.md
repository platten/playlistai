...
A later fresh, unstitched verification on `multichannel/v46` passed the same unchanged fixture **40/40** with seed `42`, empty taste, no count override, and the same installed dependency classes. All cases completed within 120 seconds with no assertion errors or timeouts: 388 recordings total, 5–10 per prompt, 11 fulfilled and 29 honestly partial outcomes, median measured prompt time 78.3 seconds, maximum 115.1 seconds. A separate fresh pass of all four unchanged Generate-screen examples at the screen's 20-track default also passed **4/4**: output counts were 20, 11, 10, and 20; the maximum measured prompt time was 105.2 seconds. All four outcomes were partial, with unsupported characteristics still labeled rather than claimed as verified.

The follow-up run uses exact indexed artist discovery for pack profiles and sourced same-recording component tags only as *partial* support for the reviewed `ambient electronica` concept. A count-scaled soft-stop reserve keeps both ten-track instrumental journeys and twenty-track no-vocals requests within the unchanged hard deadline. Regressions cover those evidence boundaries, missing components, strict vocal screening, indexed artist lookup, and the Generate example's negative mood assertion. The private reports and isolated stores remain outside the repository; these automated results establish bounded contract correctness, not held-out listening quality.
