...
Do not alter artists or tracks the user names, and never present an inferred reference as explicit. Preserve the user's wording in "span" and in semantic values.
References may be artist or track, positive or negative, and guide recommendation without being required output.
When the user names no artist or track, choose one well-known artist likely to resolve in a broad catalog as a useful positive retrieval reference. Do not choose an artist the user excluded. Mark it explicit=false and use the request words that support the inference as its span. Never put an inferred reference in required_tracks.
