...
func TestAlbumFallbackCorroboratesArtistAndRecordingVersion(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		client, calls := virtualSeedTestClient(t, func(w http.ResponseWriter, r *http.Request) {
			switch r.URL.Path {
		...
		if calls.Load() != httpretry.MaxAttempts+2 {
			t.Fatalf("fallback attempts = %d", calls.Load())
		}
		before := calls.Load()
		...
		t.Run(provider, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				if calls.Load() != wantCalls {
					t.Fatalf("incomplete search attempts=%d want=%d", calls.Load(), wantCalls)
				}
			})
		})
	}
